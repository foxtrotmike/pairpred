...
class getExamplesDBD:
    ...
    """
    def __init__(self,idir,pdthr=6.0,mdthr=8.0):
        """
        ...
        self.pdthr=pdthr
        self.dir=idir
        ufiles=glob.glob(os.path.join(idir,"*_l_u.pdb"))#[:3]
        self.Pex=dict()
        self.Nex=dict()
        self.Plr=dict()
        self.dict_gdf=dict() #stores the gdf values for slack rescaling for examples close to the pdthr, others have gdf values equal to 1.0
        for f in ufiles:            
            (_,cname,_)=getFileParts(f)
            cname=cname[:-4]
            print "############################ Processing: "+cname
            try:
                (Pl,Pr,P,lenL,lenR)=getPosex(idir,cname,pdthr=pdthr,mdthr=mdthr)                        
            except:                
                print "!!!!!!!!!!!!!!!!!!!!!!!! Error processing: "+cname
                continue
            self.dict_gdf[cname]=dict_gdf
            self.Nex[cname]=[]
...
if __name__=="__main__":    
    pdbpath='/s/chopin/b/grad/minhas/Desktop/DBD4N/DBD4/'#'./DBD4N/DBD4/'#
    E=getExamplesDBD(pdbpath,pdthr=6.0,mdthr=8.0)
    print "-----------Choosing Negative Examples-----------"
    E.selectNegEx(p=100.0,M='+-')
    print "---Computing distances to nearest examples of opposite sign----"
    E.attachD2N()
    E.save('EP_6N.lbl.pkl')
