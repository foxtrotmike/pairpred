# -*- coding: utf-8 -*-
"""
Created on Tue Oct 22 09:39:14 2013

@author: fayyaz
"""
from myPDB import *
#Find ids of single chain complexes
...
for cid in incids:
    ...
    if len(np.unique([r.full_id[2] for r in L.R]))>1:
        continue
    R=myPDB.loader(bdir+'/'+cid+'_r_u.pdb.pkl')
    if len(np.unique([r.full_id[2] for r in R.R]))>1:
        continue
    singles[cid]=len(L.R)*len(R.R)