...
from myPDB import *
from getExamplesDBD_breakup import getExamplesDBD
from protKernel import *
from myKernelData import *
from pairFeatures import computecPFKdict
import cPickle
try:
...
def dbdK2pwK(cKdict,Sr,Sc=None,justdiag=False,pfKdict=None):
    """
    ...
    Input:
            ...
            pfKdict: Pairwise or other kernels dictionary, when not none, the other kernel is added to the existing kernel
    Return:
    ...
    for aidx,cida in enumerate(dr.keys()):    
        ...
        for cidb in dc.keys()[st:]:    
            ...
            #pdb.set_trace()
            if pfKdict is not None:
                tK2=pfKdict[(_cida,_cidb)]
                assert [Sc[i][1] for i in dc[_cidb][3]]==tK2.Bidx 
                assert [Sr[i][1] for i in dr[_cida][3]]==tK2.Aidx                
                kab=(kab+tK2.K)/2.0
            if _cida!=cida:
                kab=kab.T
...
class dbdKernel:
    ...
    """
    def __init__(self,pdbpklpath,E0,ofname=None,incids=None,**kwargs):
        comm = MPI.COMM_WORLD
        myid = comm.Get_rank()
        ...
        pfKdict=None
        calcpfKdict=False
        if 'calcpfK' in kwargs:
            calcpfKdict=kwargs['calcpfK']  
        if calcpfKdict:
            pfKdict=computecPFKdict(KK,pdbpklpath,E,comm=comm,myid=myid,nprocs=nprocs,**kwargs)      
        if myid==0:            
            self.__dbdK2pwK__(KK,E,cKdict,pfKdict=pfKdict)
            #pdb.set_trace()
            #self.setCvals()
    ...
    def __repr__(self):
        s="PAIRpred Kernel (dbdKernel) Object"
        s+='\n\t'+str(self.E)
        s+='\n\t Kernel dimensions:'+str(self.K.shape)
        return s
    ...
    @classmethod  
    def loadNonKernelData(self,pklfile):
        """
...
def getKernelData(K,S,*arg):
    """
    Return the myKernelData object (convert to pyml style kernel)
    """
    #pdb.set_trace()

    mK=myKernelData(K,patternID=zip(*zip(*S)[:2]),labels=list(zip(*S)[-1]))
    return mK
...
class complexKernel:
    ...
    def __init__(self,cidA,cidB,pdbpklpath,E,aAll=0,bAll=0):
        ...
        lAname=os.path.join(pdbpklpath,cidA[0]+'.pdb.pkl')
        rAname=os.path.join(pdbpklpath,cidA[1]+'.pdb.pkl')
        lBname=os.path.join(pdbpklpath,cidB[0]+'.pdb.pkl')
        rBname=os.path.join(pdbpklpath,cidB[1]+'.pdb.pkl') 
...
def getValidE(E0,pdbpklpath,cids=None):
    ...
    s=set()
    for k in K:        
        if len(glob.glob(os.path.join(pdbpklpath,k[0]+'.pdb.pkl')))==1 and len(glob.glob(os.path.join(pdbpklpath,k[1]+'.pdb.pkl')))==1:
            s.add(k)
...
if __name__=="__main__":    
    pdbpklpath='../../DBD4CSPKL/PKL/' #'/s/chopin/b/grad/minhas/Desktop/DBD4CNS/PKL/'#
    exfname=exfname=pdbpklpath+'ENS_15_35_50.lbl.pkl'
    ofname='dbdKernel.dbk.pkl'
    calcpfK=False
    msadir='./ecstats_rand2'
    E=getExamplesDBD.loader(exfname)
    ccids=E.Pex.keys()
    incids=['1PVH','1A2K','1T6B','1BKD','2IDO','1VFB']
    ccids=[k for k in E.Pex if k[0][:4] in incids]
...
#    incids=f3+f4
    ...
    #incids=E.Pex.keys()[0:10]
    #incids=['1A2K','1ACB','1AHW','1AY7']

    
#    print E.Pex.keys()
    dK=dbdKernel(pdbpklpath,E,ofname,ccids,calcpfK=calcpfK,msadir=msadir)
